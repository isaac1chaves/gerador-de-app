...
function buscar(rawOverride = null) {
  try {
    ...
    if (listasEncontradas.length >= 2) return void mostrarResultado(raw, cidadeBruta, SEARCH_STATUS.DUPLICATE, [], null, listasEncontradas, normToOriginal.get(keyRaw) || cidadeBruta, undoInfo);
    if (inAna) return void mostrarResultado(raw, cidadeBruta, SEARCH_STATUS.ANAPOLIS, [], null, [], normToOriginal.get(keyRaw) || cidadeBruta, undoInfo);
    if (inBra) return void mostrarResultado(raw, cidadeBruta, SEARCH_STATUS.BRASILIA, [], null, [], normToOriginal.get(keyRaw) || cidadeBruta, undoInfo);
    if (inCob) return void mostrarResultado(raw, cidadeBruta, SEARCH_STATUS.ACCEPTED, [], null, [], normToOriginal.get(keyRaw) || cidadeBruta, undoInfo);
    ...
    mostrarResultado(raw, cidadeBruta, SEARCH_STATUS.NOT_FOUND, sugs, null);
  } catch (err) {
